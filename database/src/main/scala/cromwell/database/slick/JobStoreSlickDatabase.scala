...
trait JobStoreSlickDatabase extends JobStoreSqlDatabase {
  ...
  private def addJobStoreEntry(jobStoreEntry: JobStoreEntry, jobStoreSimpletonEntries: Iterable[JobStoreResultSimpletonEntry])
                              (implicit ec: ExecutionContext): DBIO[Unit] = {
    for {
      insertJobStoreEntry <- dataAccess.jobStoreAutoInc += jobStoreEntry
      jobStoreEntryId = insertJobStoreEntry.jobStoreId.get
      entries = jobStoreSimpletonEntries map { _.copy(jobStoreEntryId = jobStoreEntryId) }
      _ <- dataAccess.jobStoreResultSimpletonAutoInc ++= entries
    } yield ()
  }

  override def addJobStoreEntries(entries: Iterable[(JobStoreEntry, Iterable[JobStoreResultSimpletonEntry])])(implicit ec: ExecutionContext): Future[Unit] = {
