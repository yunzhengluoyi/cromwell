...
class SlickDataAccess(databaseConfig: Config) extends DataAccess {
  ...
  override def createWorkflow(workflowDescriptor: WorkflowDescriptor,
                              ...
                              scopes: Traversable[Scope])(implicit ec: ExecutionContext): Future[Unit] = {
  ...
  private def insertEmptyExecutionInfos(insertResult: this.dataAccess.executionsAutoInc.SingleInsertResult): DBIO[_] = {

    def buildInsert(key: String) = {
      dataAccess.executionInfosAutoInc += new ExecutionInfo(insertResult.executionId.get, key, None)
    }

    val inserts = Seq("PID") map buildInsert
  ...
  private def toScopeAction(workflowExecution: WorkflowExecution)
    ...
    // Insert an execution row
      ...
      _ <- insertEmptyExecutionInfos(executionInsert)
    } yield ()
  }
  ...
  /**
    ...
    */
  override def updateCallInputs(workflowId: WorkflowId, key: BackendCallKey, callInputs: cromwell.engine.CallInputs)
  ...
  private def setStatusAction(workflowId: WorkflowId, scopeKeys: Traversable[ExecutionDatabaseKey],
    ...
    val findResultsClonedFromId = DBIO.successful(callStatus.resultsClonedFrom flatMap { _.execution.executionId } )
