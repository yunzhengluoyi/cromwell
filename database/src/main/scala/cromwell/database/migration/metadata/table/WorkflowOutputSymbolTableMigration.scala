...
class WorkflowOutputSymbolTableMigration extends SymbolTableMigration {
  override protected def selectQuery: String = """
     SELECT SYMBOL.SCOPE, SYMBOL.IDX, SYMBOL.NAME, IO, WDL_TYPE, WDL_VALUE, WORKFLOW_EXECUTION_UUID
       |FROM SYMBOL
       |  JOIN WORKFLOW_EXECUTION ON SYMBOL.WORKFLOW_EXECUTION_ID = WORKFLOW_EXECUTION.WORKFLOW_EXECUTION_ID
       |WHERE SYMBOL.IO = 'OUTPUT' AND SYMBOL.IDX = -1 AND SYMBOL.REPORTABLE_RESULT = 1;
       |""".stripMargin
