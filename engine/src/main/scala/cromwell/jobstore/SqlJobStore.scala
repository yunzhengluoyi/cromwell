...
class SqlJobStore(sqlDatabase: JobStoreSqlDatabase) extends JobStore {
  ...
  private def toDatabase(jobCompletion: (JobStoreKey, JobResult)): (JobStoreEntry, Iterable[JobStoreResultSimpletonEntry]) = {
    jobCompletion match {
      case (key, JobResultSuccess(returnCode, jobOutputs)) =>
        ...
        val simpletons = jobOutputs.mapValues(_.wdlValue).simplify.map {
          s => JobStoreResultSimpletonEntry(s.simpletonKey, s.simpletonValue.valueString, s.simpletonValue.wdlType.toWdlString, -1) }
        (entry, simpletons)
