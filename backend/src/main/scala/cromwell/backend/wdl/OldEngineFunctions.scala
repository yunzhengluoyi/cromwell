...
import cromwell.core.{OldCallContextOld, OldWorkflowContext}
...
trait OldCallEngineFunctions extends WdlStandardLibraryImpl {
  protected def stdout(context: OldCallContextOld): Try[WdlFile] = Success(WdlFile(context.stdout))
  protected def stderr(context: OldCallContextOld): Try[WdlFile] = Success(WdlFile(context.stderr))
}
